...
"""
simulate random walk in the graph
"""
...
import pandas as pd
from ge.random_walker.alias_sample import create_alias_table, alias_sample
...
class RandomWalker:
        ...
        while len(walk) < walk_length:
            ...
            if len(cur_nbrs) > 0:
                if len(walk) == 1:
                    ...
                    prev = walk[-2]
                    edge = (prev, cur)
                    walk.append(cur_nbrs[alias_sample(alias_edges[edge][0], alias_edges[edge][1])])
            else:
                break
    ...
    def get_alias_edge(self, t, v):
        ...
        :param t: previous node t
        :param v: current node v
        :return: transition probas table, consist of next random node's (every neighbor of v) probability.
        """
        G = self.G
        p = self.p
        q = self.q
        ...
        n_next_node = len(unnormalized_probs)

        normalized_probs = [float(u_prob) / norm_const * n_next_node for u_prob in unnormalized_probs]
    ...
    def preprocess_transition_probs(self):
        ...
        for node in G.nodes():
            ...
            norm_const = sum(unnormalized_probs)
            n_neighbors = len(unnormalized_probs)
            normalized_probs = [float(u_prob) / norm_const * n_neighbors for u_prob in unnormalized_probs]
...
class BiasedWalker(object):
    """
    Biased walker used in Struc2vec.
    """
