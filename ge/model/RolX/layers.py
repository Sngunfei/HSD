...
class Factorization:
    ...
    def __init__(self, args, user_size, feature_size):
        ...
        self.embedding_node = tf.Variable(tf.random_uniform([self.user_size, self.args.dim],
                                            -0.1/self.args.dim, 0.1/self.args.dim))

        self.embedding_feature = tf.Variable(tf.random_uniform([self.feature_size, self.args.dim],
                                            -0.1/self.args.dim, 0.1/self.args.dim))
